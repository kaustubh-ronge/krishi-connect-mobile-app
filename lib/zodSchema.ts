...
export const farmerSchema = z.object({
  ...
  lat: z.preprocess((val) => val === "" || val === null || val === undefined ? undefined : parseFloat(val as string), z.number().optional()),
  lng: z.preprocess((val) => val === "" || val === null || val === undefined ? undefined : parseFloat(val as string), z.number().optional()),
  primaryProduce: z.array(z.string()).min(1, "Please select at least one primary produce"),
  usagePurpose: z.enum(["buy", "buy_and_sell"], { message: "Please select a profile purpose" }),
...
export const deliverySchema = z.object({
  ...
  radius: z.preprocess((val) => parseFloat(val as string), z.number().min(1, "Minimum service radius is 1 KM")),
  pricePerKm: z.preprocess((val) => parseFloat(val as string), z.number().min(0, "Price per KM cannot be negative")),

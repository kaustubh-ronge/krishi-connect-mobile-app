...
import { getRouteParam, unwrapData } from '@/lib/apiHelpers';
...
export default function EditListingScreen() {
  ...
  const listingId = getRouteParam(useLocalSearchParams<{ id: string }>().id);
  ...
  React.useEffect(() => {
    ...
    const fetchListing = async () => {
      try {
        const res = await api.get(`mobile/v1/products?id=${listingId}`);
        const data = unwrapData<any>(res);
        if (data) {
          setProductName(data.productName || "");
          setSelectedCategory(PRODUCE_CATEGORIES.includes(data.category) ? data.category : "Other");
          if (!PRODUCE_CATEGORIES.includes(data.category)) setCustomCategory(data.category || "");
          setQualityGrade(data.qualityGrade || "Grade A (Premium)");
          setTags(data.variety || "");
          setShelfLife(data.shelfLife || "");
          setShelfLifeStartDate(data.shelfLifeStartDate ? new Date(data.shelfLifeStartDate).toISOString().split('T')[0] : "");
          setStock(data.availableStock?.toString() || "");
          setUnit(UNIT_OPTIONS.includes(data.unit) ? data.unit : "Other");
          if (!UNIT_OPTIONS.includes(data.unit)) setCustomUnit(data.unit || "");
          setPrice(data.pricePerUnit?.toString() || "");
          setMinOrderQuantity(data.minOrderQuantity?.toString() || "");
          setHarvestDate(data.harvestDate ? new Date(data.harvestDate).toISOString().split('T')[0] : "");
          setImages(data.images || []);
          setWhatsappNumber(data.whatsappNumber || "");
          setDescription(data.description || "");
          setDeliveryCharge(data.deliveryCharge?.toString() || "0");
          setDeliveryChargeType(data.deliveryChargeType || "per_unit");
          setMaxDeliveryRange(data.maxDeliveryRange?.toString() || "");
        }
      } catch (error: any) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await api.put(`mobile/v1/seller/listings?id=${listingId}`, validationData);
        ...
        {currentStep < 4 ? (
          ...
          <TouchableOpacity className="flex-row items-center justify-center bg-green-500 p-4 rounded-2xl gap-2" onPress={handleSubmit} disabled={loading}>
            {loading ? <ActivityIndicator color="white" /> : (
              <>
                <Text className="text-white text-base font-bold">Update Listing</Text>
